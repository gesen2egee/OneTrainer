    ...
    def _flow_matching_losses(
        ...
        if 'timestep' in data:
            match config.loss_weight_fn:
                ...
                case LossWeight.MIN_SNR_GAMMA | LossWeight.DEBIASED_ESTIMATION | LossWeight.P2:
                    # For flow matching, approximate SNR from sigma:
                    #   SNR = 1 / sigma^2
                    sigma_values = self.__sigmas[data['timestep']].to(losses.device)
                    sigma_values = torch.clamp(sigma_values, min=1e-4)
                    snr = 1.0 / (sigma_values ** 2)

                    # Flow matching is conceptually closest to v-prediction weighting behavior.
                    v_pred = True

                    if config.loss_weight_fn == LossWeight.MIN_SNR_GAMMA:
                    ...
                    elif config.loss_weight_fn == LossWeight.DEBIASED_ESTIMATION:
                        weight = snr.clone()
                        torch.clip(weight, max=1.0e3, out=weight)
                        if v_pred:
                            weight += 1.0
