    ...
    match config.optimizer.optimizer:
        ...
        case Optimizer.ADAMW_ADV:
            ...
            optimizer = AdamW_adv(
                ...
                alpha_grad=optimizer_config.alpha_grad if optimizer_config.alpha_grad is not None else 100.0,
                vector_reshape=optimizer_config.vector_reshape if optimizer_config.vector_reshape is not None else False,
                beta2_min=optimizer_config.beta2_min if optimizer_config.beta2_min is not None else 0.9,
                ema_alpha=optimizer_config.ema_alpha if optimizer_config.ema_alpha is not None else 0.95,
                tiny_spike=optimizer_config.tiny_spike if optimizer_config.tiny_spike is not None else 1e-9,
                dual_adam=optimizer_config.dual_adam if optimizer_config.dual_adam is not None else True,
                dual_adam_xi=optimizer_config.dual_adam_xi if optimizer_config.dual_adam_xi is not None else 0.0,
                dual_adam_alpha_max=optimizer_config.dual_adam_alpha_max if optimizer_config.dual_adam_alpha_max is not None else 1.0,
                use_skipupdate=optimizer_config.use_skipupdate if optimizer_config.use_skipupdate is not None else False,
                use_magma=optimizer_config.use_magma if optimizer_config.use_magma is not None else True,
                mask_p=optimizer_config.mask_p if optimizer_config.mask_p is not None else 0.5,
                magma_tau=optimizer_config.magma_tau if optimizer_config.magma_tau is not None else 2.0,
                magma_ema=optimizer_config.magma_ema if optimizer_config.magma_ema is not None else 0.9,
                mask_1d_params=optimizer_config.mask_1d_params if optimizer_config.mask_1d_params is not None else False,
            )
